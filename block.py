...
class ResNet18(nn.Module):
    def __init__(self, num_classes=1000):
        super().__init__()
        # 输入图像通道数 c1=3
        self.stem = ResNetLayer(c1=3, c2=64, is_first=True, n=1)

        # ResNet18 的四个阶段，每个阶段有2个残差块
        self.layer1 = ResNetLayer(c1=64, c2=64, s=1, n=2)   # 不下采样
        self.layer2 = ResNetLayer(c1=64, c2=128, s=2, n=2)  # 下采样
        self.layer3 = ResNetLayer(c1=128, c2=256, s=2, n=2) # 下采样
        self.layer4 = ResNetLayer(c1=256, c2=512, s=2, n=2) # 下采样
...
class Bottlenecke(nn.Module):
    """Bottleneck-e: 3x3 DWConv -> 1x1 Conv -> 3x3 Conv"""
...
class C3k2e(C3k2):
    """C3k2_e: Faster CSP Bottleneck with Bottleneck-e"""
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        # 替换卷积分支为 Bottleneck-e 或 C3k-e
        self.m = nn.ModuleList(
...
class AAttn(nn.Module):
    ...
    Splits the feature map into five regions (4 corners + center),
    computes attention in each region, then merges them.
    """
    def __init__(self, dim: int, num_heads: int, area: int = 1):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
        self.area = area  # 仅为了接口兼容
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        regions = [
            ...
            x[:, :, h_half//2:h_half//2+h_half, w_half//2:w_half//2+w_half]  # center
        ]
        ...
        for r in regions:
            ...
            out = out.permute(0, 3, 1, 2).contiguous()  # (B_r, N, num_heads, head_dim)
            out = out.view(B_r, C_r, H_r, W_r)          # reshape回原区域
